...
AInventoryCppCharacter::AInventoryCppCharacter()
{
	...
	PlayerInventory = CreateDefaultSubobject<UInventoryComponent>(
...
void AInventoryCppCharacter::NotifyControllerChanged()
{
...
void AInventoryCppCharacter::SetupPlayerInputComponent(
	UInputComponent* PlayerInputComponent)
{
		...
		UEnhancedInputComponent>(PlayerInputComponent))
	{
		...
		EnhancedInputComponent->BindAction(InteractAction,
		                                   ...
		                                   &AInventoryCppCharacter::BeginInteract);
		EnhancedInputComponent->BindAction(InteractAction,
		...
		EnhancedInputComponent->BindAction(MenuAction, ETriggerEvent::Started,
		                                   this,
		                                   &AInventoryCppCharacter::ToggleMenu);

		// Moving
		EnhancedInputComponent->BindAction(MoveAction, ETriggerEvent::Triggered,
		                                   this, &AInventoryCppCharacter::Move);

		// Looking
		EnhancedInputComponent->BindAction(LookAction, ETriggerEvent::Triggered,
		                                   this, &AInventoryCppCharacter::Look);
	}
	else
	{
...
void AInventoryCppCharacter::Move(const FInputActionValue& Value)
{
	// input is a Vector2D
...
void AInventoryCppCharacter::Look(const FInputActionValue& Value)
{
	// input is a Vector2D
...
void AInventoryCppCharacter::Tick(float DeltaSeconds)
{
	Super::Tick(DeltaSeconds);

	if (GetWorld()->TimeSince(InteractionData.LastInteractionCheckTime) >
...
void AInventoryCppCharacter::PerformInteractionCheck()
{
	...
	// prevent interaction behind actor.
	float LookDirection = FVector::DotProduct(GetViewRotation().Vector(),
	...
	if (LookDirection > 0)
	{
		...
		if (GetWorld()->LineTraceSingleByChannel(
			TraceHit, TraceStart, TraceEnd, ECC_Visibility, QueryParams))
		{
			if (TraceHit.GetActor()->GetClass()->ImplementsInterface(
...
void AInventoryCppCharacter::FoundInteractable(AActor* NewInteractable)
{
	if (IsInteracting())
	{
		EndInteract();
...
void AInventoryCppCharacter::NoInteractableFound()
{
	if (IsInteracting())
	{
...
void AInventoryCppCharacter::BeginInteract()
{
	// verify nothing has changed with the interactable state
	PerformInteractionCheck();
	...
	if (InteractionData.CurrentInteractable)
	{
		if (IsValid(TargetInteractable.GetObject()))
		{
			...
			if (FMath::IsNearlyZero(
				TargetInteractable->InteractableData.InteractionDuration, 0.1f))
			{
				Interact();
			}
			else
			{
				GetWorldTimerManager().SetTimer(TimerHandle_Interaction, this,
				                                &AInventoryCppCharacter::Interact,
				                                TargetInteractable->
				                                InteractableData.
...
void AInventoryCppCharacter::EndInteract()
{
	GetWorldTimerManager().ClearTimer(TimerHandle_Interaction);
...
void AInventoryCppCharacter::Interact()
{
	GetWorldTimerManager().ClearTimer(TimerHandle_Interaction);
...
void AInventoryCppCharacter::UpdateInteractionWidget() const
{
	if (IsValid(TargetInteractable.GetObject()))
	{
		HUD->UpdateInteractionWidget(&TargetInteractable->InteractableData);
...
void AInventoryCppCharacter::DropItem(UItemBase* ItemToDrop,
                                      const int32 QuantityToDrop)
{
	if (PlayerInventory->FindMatchingItem(ItemToDrop))
	{
		...
		const int32 RemovedQuantity = PlayerInventory->RemoveAmountOfItem(
			ItemToDrop, QuantityToDrop);

		APickup* Pickup = GetWorld()->SpawnActor<APickup>(
			APickup::StaticClass(), SpawnTransform, SpawnParams);
